...
class Q4ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        textFileName = "q4_" + String(userID!) + ".txt"
        createTextFile()
    }
    ...
    func recordData() {
        ...
        //imageをアプリ内に保存
        let fileName = "q4_" + String(userID!) + "_" + String(solution) + ".png"
    ...
    @IBAction func endQuestion(_ sender: Any) {
        ...
        let defaultAction: UIAlertAction = UIAlertAction(title: "はい", style: UIAlertAction.Style.default, handler:{
            (action: UIAlertAction!) -> Void in
            self.performSegue(withIdentifier: "to5", sender: nil)
        })
        // キャンセルボタン
        let cancelAction: UIAlertAction = UIAlertAction(title: "キャンセル", style: UIAlertAction.Style.cancel, handler:{
            (action: UIAlertAction!) -> Void in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let q4ViewController = segue.destination as! Q5ViewController
        q4ViewController.userID = userID
        q4ViewController.images = images
