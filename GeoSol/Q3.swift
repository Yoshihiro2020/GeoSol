...
class Q3ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        textFileName = "q3_" + String(userID!) + ".txt"
        createTextFile()
    }
    ...
    func recordData() {
        ...
        //imageをアプリ内に保存
        let fileName = "q3_" + String(userID!) + "_" + String(solution) + ".png"
    ...
    @IBAction func endQuestion(_ sender: Any) {
        ...
        let defaultAction: UIAlertAction = UIAlertAction(title: "はい", style: UIAlertAction.Style.default, handler:{
            (action: UIAlertAction!) -> Void in
            self.performSegue(withIdentifier: "to4", sender: nil)
        })
        // キャンセルボタン
        let cancelAction: UIAlertAction = UIAlertAction(title: "キャンセル", style: UIAlertAction.Style.cancel, handler:{
            (action: UIAlertAction!) -> Void in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        let q3ViewController = segue.destination as! Q4ViewController
        q3ViewController.userID = userID
        q3ViewController.images = images
